...
cd /home/flight/crontab/hit_rate
...
hive -e "select  get_json_object(f.exts,'$.route') as f_route, get_json_object(f.exts,'$.departDate') as f_departDate  ,sum(get_json_object(f.exts,'$.total')) as f_total ,sum(get_json_object(f.exts,'$.hasSouthern')) as f_hasSouthern ,sum(get_json_object(f.exts,'$.hasNoSouthern')) as f_hasNoSouthern from flight_log.flight_request_log f where time>'$before' and time < '$now' and  dt = '$today' and log_type='SouthernFlag'   group by  get_json_object(f.exts,'$.route') ,get_json_object(f.exts,'$.departDate')  having f_hasNoSouthern  >= 1  order by f_route ,f_departDate  " > hit_rate.log
hive -e "select  get_json_object(f.exts,'$.route') as f_route,get_json_object(f.exts,'$.channel') as f_channel , get_json_object(f.exts,'$.date') as f_departDate,round(sum(get_json_object(f.exts,'$.hit'))/( sum(get_json_object(f.exts,'$.hit'))+sum(get_json_object(f.exts,'$.missHit')) ),2 )*100 from flight_log.flight_request_log f  where dt = '$today' and log_type='HitRate'     group by  get_json_object(f.exts,'$.channel'),get_json_object(f.exts,'$.route') ,get_json_object(f.exts,'$.date') order by   f_route,f_channel,f_departDate" > hit_rate.log
...
# 同一个id只取时间最新的记录
echo '路线&nbsp;&nbsp;&nbsp;&nbsp;渠道&nbsp;&nbsp;&nbsp;&nbsp;起飞时间&nbsp;&nbsp;&nbsp;&nbsp;命中率百分比</br>' > tosend_header.log

cat hit_rate.log > tosend_content.log
