package com.javaapi.test.test.javafeature.version8.streams.streammap;
...
//        System.out.println("collect = " + JSON.toJSONString(collect));
    ...
    public void testTwoNestMapOther() {
        ...
        BiConsumer<List<String>, List<String>> listAccumulator = List::addAll;

        BinaryOperator<List<String>> listCombiner = (left, right) -> {
            left.addAll(right);
            return left;
        };
        Collector<List<String>, List<String>, List<String>> listCollector = Collector.of(listSupplier, listAccumulator, listCombiner);

        Collector<Person, ?, List<String>> mapping = Collectors.mapping(goDateMapper, listCollector);
